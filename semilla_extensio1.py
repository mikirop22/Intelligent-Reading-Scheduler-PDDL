...
def generate_random(num_libros_quiere_leer, num_libros_catalogo):
    ...
    with open("random_problem.pddl", "w") as file:
        ...
        predecesors=[]
        for _ in range(random.randint(0, num_libros_catalogo)): #hi haura 0 o mes llibres amb predecesor
            ...
            if ((libro1, libro2) and (libro2, libro1)) not in predecesors: # mirem que un llibre no pot ser predecessor del mateix llibre si l'altre ho és del mateix
                file.write(f"    (predecesor {libro1} {libro2})\n")
                predecesors.append((libro1, libro2))

        #paralels=[]
        #for _ in range(random.randint(0, num_libros_catalogo//2)): #hi haura 0 o mes llibres paralels
         #   libro1 = random.choice(libros_quiere_leer + libros_catalogo)
